...
const page = () => {
        ...
        salidas?.batches?.forEach((batch: Batch) => {
            batch.containers?.forEach((container: Container) => {
                container.batchItems?.forEach((item: BatchItem) => {
                    ...
                    // Crear una sola fila por batch item, sin importar cuántos pedidos tenga
                    const pedidosAsignados = item.batchItemOrderItems && item.batchItemOrderItems.length > 0
                        ? item.batchItemOrderItems.map(bioi => bioi.orderItem.order.orderNumber).join(', ')
                        : 'Sin asignar'

                    const cantidadTotalAsignada = item.batchItemOrderItems && item.batchItemOrderItems.length > 0
                        ? item.batchItemOrderItems.reduce((total, bioi) => total + bioi.quantity, 0)
                        : 0
                    ...
                    tableRows.push({
                        ...
                        cantidad: cantidadTotalAsignada, // Mostrar la cantidad total asignada a todos los pedidos
    ...
    }, [salidas, searchTerm, filtroFecha])
